---
title: "Mainnet cutover runbook"
description: "The deliberate, reversible, audited procedure to enable Stellar pubnet for the Buckspay SDK and facilitator."
---

<Warning>
  **Invariant:** nothing touches pubnet until the GO/NO-GO gate passes, and every step has a
  rollback. Mainnet is OFF by default — the cutover is "turn the deliberate opt-in on, watched",
  and the rollback is "turn it back off."
</Warning>

**Owner:** buckspay eng lead · **On-call:** the rotation named in the team runbook (PagerDuty) ·
**Security sign-off:** per [Audit prep](/production/security/audit-prep).

---

## Pre-flight checklist

Every row maps to a concrete verification command. **All must be green before the GO/NO-GO gate.**
...
- **Post-incident:** a written review within 48h (timeline, root cause, the guard that should have
  caught it, the follow-up). The relayer-trust + sponsor-exposure surfaces are in the
  [threat model](/production/security/threat-model).
...
3. **MAINTAINER (user)** — run `pnpm changeset version` (computes the lockstep bump + CHANGELOGs —
   ...
   never runs version/publish** — it is outward, irreversible, and needs npm auth (see
   [Versioning and migration](/production/versioning-and-migration)).
