...
description: "Which wasm Buckspay deploys, where it comes from, how to reproduce it, and what that means for the audit scope."
---
...
`scripts/check-pin-parity.sh`, and this document.
...
- The hash is the **sha256 of the wasm bytes**, so it is **network-independent** - identical on
  ...
  see [Pin & install](#pin--install) below.
...
- **Residual risk / re-scope trigger:** adopting a future "real OZ Smart Account release" (or any
  wasm change) **re-scopes** this - it re-runs the build + hash procedure, re-pins, updates this
  doc, and re-audits the new bytes.
...
- **Facilitator config:** `facilitator/src/stellarContract.ts` - `OZ_SMART_ACCOUNT_WASM_HASH`
  ...
  `packages/accounts/test/pin-parity.test.ts`) asserts the SDK pin === the facilitator's
...
- **Per-network install:** the wasm is installed once per network via `uploadContractWasm`.
  ...
  install is `facilitator/scripts/install-wasm-pubnet.ts` - gated by
