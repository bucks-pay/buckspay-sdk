---
title: "Key handling"
description: "Where every key and secret may live, where it must not, and the operational rules that enforce the boundary."
---
...
- **Rotate** the sponsor secret and the facilitator API key on a schedule; treat anything
  that ever entered git history as compromised (rotate immediately). See the
  [secret rotation runbook](/production/security/secret-rotation).
