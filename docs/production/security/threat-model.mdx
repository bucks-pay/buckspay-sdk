---
title: "Threat model"
description: "STRIDE-style analysis of the six security surfaces in the Buckspay SDK."
---
...
secp256r1). `BuckspaySigner` returns **only** public keys + signatures — there is no
...
**Status: mitigated** (single-provider risk **accepted** for v1; relayer neutrality is planned for a future release).
...
rotation + spend caps tracked in [Key handling](/production/security/key-handling).
...
social login). See [Audit prep](/production/security/audit-prep).
