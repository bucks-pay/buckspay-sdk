 ...
 * sprint-0/03 spike proved the unsigned entry is identical across models; only the signer differs.
 *
 ...
 *
 * Encoding ported verbatim from `spikes/sp2-multicall/src/multicall.ts` (`buildBatchTransferArgs`):
 * `transfers` is an ScVec of 2-tuples, each an ScVec `[Address, i128]` (how soroban-sdk represents
 * `Vec<(Address, i128)>`). Byte-checked against `spikes/sp2-multicall/fixtures/multicall-batch.json`.
 *
